...
export async function extractMediaFromZip(zip: JSZip): Promise<MediaStore> {
  ...
  const storyPaths = allFiles.filter(
    (f) => normPath(f).includes('media/stories/') && isSupportedMedia(f) && !zip.files[f]?.dir
  );
  ...
  console.log('[InstaInsight][MediaExtraction] Grouped stories months:', Object.keys(storiesGrouped));
  console.log('[InstaInsight][MediaExtraction] Grouped posts months:', Object.keys(postsGrouped));
